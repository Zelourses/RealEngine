...
void Sandbox2D::onAttach() {
	...
	spriteSheet       = Real::Texture2D::create("assets/textures/RPGpack_sheet_2X.png");
	barrelSprite      = Real::SubTexture2D::createFromCoods(spriteSheet, {8, 2}, {128, 128});
}
...
void Sandbox2D::onUpdate(Real::Timestep ts) {
	...
	Real::Renderer2D::drawQuad({0.0, 0.0, 0.0}, {0.5, 0.5}, barrelSprite);
	...
	for (float y = -5.0f; y < 5.0f; y += 0.5f) {
		for (float x = -5.0f; x < 5.0f; x += 0.5f) {
			...
			Real::Renderer2D::drawQuad({x, y, -0.1f}, {0.45f, 0.45f}, color);
		}
	}
	Real::Renderer2D::endScene();
}
